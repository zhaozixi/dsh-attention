...
/**
 * Total preset count — used by tests and the engine's cycle sizing.
 * @type {number}
 */
export const PRESET_COUNT = PRESET_ACTIVITIES.length;

/**
 * Look up a preset activity by id.
 * @param {string} id - activity id.
 * @returns {Activity|undefined} the matched activity, if any.
 */
export function findPresetActivity(id) {
  return PRESET_ACTIVITIES.find((activity) => activity.id === id);
}

/**
 * Filter presets to one category.
 * @param {ActivityCategory} category - category key.
 * @returns {Activity[]} matching presets.
 */
export function presetsByCategory(category) {
  return PRESET_ACTIVITIES.filter((activity) => activity.category === category);
}
