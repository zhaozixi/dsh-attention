 ...
 * cadence and emits an activity suggestion (walking a shuffled pool,
 * never repeating an id within a session until the pool is exhausted).
 * When every task settles, the engine emits a session report and stops.
 *
 * All scheduling lives here (host side); the client panel is a passive
 * renderer of the engine's emitted events.
 */

import { PRESET_ACTIVITIES } from "./activities.js";
...
export class ReminderEngine {
  ...
  _handleStart(task) {
    ...
    // Match strictly on kind AND label — a missing label must not collapse
    // every task of the same kind into one window.
    const existing = this.activeTasks.findIndex(
      (active) => active.kind === task.kind
  ...
  _handleEnd(task) {
    ...
    if (index === -1) {
      // No tracked start matches this settling event (e.g. a task ended
      // before its start was observed, or a stale duplicate edge). Never
      // drop an unrelated running window — that would close a live task.
      return;
    }
