...
function loadActivities() {
  try {
    ...
    if (Array.isArray(parsed?.activities)) {
      // Validate every persisted row; drop rows that no longer conform so a
      // hand-edited or stale file cannot crash the suggestion engine.
      const valid = parsed.activities
        .map((row) => normalizeActivity(row))
        .filter((row) => row !== undefined);
      return valid.length > 0 ? valid : PRESET_ACTIVITIES;
    }
          ...
          handler: async (req, res) => {
            ...
            if (req.method === "POST") {
              ...
              const normalized = submitted
                .map((row) => normalizeActivity(row))
                .filter((row) => row !== undefined)
                // Deduplicate by id (last occurrence wins): duplicate ids
                // would inflate the pool and skew the suggestion count.
